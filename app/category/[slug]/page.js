...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    description:
      ...
      `Explore premium products in ${category.name} at OPS Udyog.`,
    openGraph: {
      title: `${category.name} | OPS Udyog`,
      description:
        category.description ||
...
export default async function CategoryPage({ params }) {
  ...
  if (!category)
    return <div className="text-center py-32">Category not found</div>;
    ...
    <div className="container mx-auto px-6 py-16">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
        {products.map((prod) => (
          <Link
            key={prod.id}
            href={`/product/${prod.slug}`} // 👈 clickable product page link
            className="group block rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-md hover:shadow-xl transition-transform duration-500 transform hover:-translate-y-1"
          >
            {/* Image */}
            {prod.images?.[0]?.src && (
            ...
            <div className="p-6 flex flex-col h-full">
              ...
              <div className="mt-4 inline-block px-6 py-3 rounded-full bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-semibold shadow text-center transition">
                View Details
              </div>
            </div>
          </Link>
