...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    // Delay floating orbs to not block LCP
    const timer = setTimeout(() => setShowOrbs(true), 1000);
  ...
  const heroSrc = isMobile ? "/hero11-mobile.jpg" : "/hero11.jpg";
  const blurSrc = isMobile ? "/hero11-mobile-blur.jpg" : "/hero11-blur.jpg";

  return (
    <>
      {/* Preload hero image for Google LCP */}
      <Head>
      ...
      <section
        id="hero-section"
        className="relative w-full min-h-screen flex items-center justify-center pt-[80px] overflow-hidden"
      >
        {/* Hero Background */}
        <motion.div
          initial={{ scale: 1.05 }}
        ...
        {/* Floating Orbs (non-critical, delayed) */}
        {showOrbs && (
          <>
            <motion.div
              animate={{ rotate: 360 }}
